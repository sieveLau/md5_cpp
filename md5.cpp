...
namespace sieve {
namespace hash {
std::array<unsigned char, 16> md5(const unsigned char *bytes,
...
std::array<unsigned char, 16> md5(const std::string &str) {
...
MD5::MD5() : buffer_(EVP_MD_CTX_new()), finish_(false) {
  EVP_DigestInit_ex(buffer_, EVP_md5(), nullptr);
}

MD5::MD5(MD5 &&another) noexcept : MD5() { swap(*this, another); }
...
// 读取bytes中的bytes_to_cal个byte来update digeset
...
/**
 * @brief 返回截止到目前的digest的hex
 *
 * @return std::string digest的hex
 */
